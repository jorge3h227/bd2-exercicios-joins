...
1 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo 
das pessoas associadas a academia, uma coluna com o logradouro e outra com o número. Utilize as tabelas Pessoa_Associada e Endereco.

USE academia;
SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', E.`Logradouro`, E.Numero
FROM
  ...
  2 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo das pessoas
associadas a academia, uma coluna com o nome do plano dessa pessoa. Utilize as tabelas Pessoa_Associada e Plano.

SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', P.`Nome_plano`
FROM
  `Pessoa_Associada` AS PA
    INNER JOIN
  Plano AS P ON PA.Plano_ID = P.PLANO_ID;

3 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo das 
pessoas associadas a academia, uma coluna com telefone e outra com o cep dessa pessoa.
Utilize as tabelas Pessoa_Associada, Telefone e Endereco.

SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', T.Telefone_Contato AS 'Telefone', E.CEP
FROM
  `Pessoa_Associada` AS PA
    INNER JOIN
...
4 - Escreva uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo das pessoas associadas a academia, 
uma coluna com telefone de contato e outra com o cep dessa pessoa. Utilize as tabelas Pessoa_Associada, Telefone e Endereco.

SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', T.Telefone_Contato AS 'Telefone de Contato', E.CEP
FROM
  `Pessoa_Associada` AS PA
    INNER JOIN
  Telefone AS T ON PA.Telefone_ID = T.Telefone_ID
...
5 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo 
de todas as pessoas associadas a academia, e uma coluna com o telefone de emergência dessa pessoa, 
caso a pessoa não tiver telefone de emergência, exiba 'PEDENTE', nomeie essa coluna como 
'Telefone de Emergência'. Utilize as tabelas Pessoa_Associada e Telefone.

SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', IFNULL(T.Telefone_Emergencia, 'PENDENTE') AS 'Telefone de Emergência'
FROM
  `Pessoa_Associada` AS PA
  INNER JOIN
  Telefone AS T ON PA.Telefone_ID = T.Telefone_ID;

  6 - Crie uma query que retorne duas coluna com os nomes das pessoas associadas que possuem o mesmo 
  endereço. Porém, não deixe que o mesmo nome se repita na mesma linha.


SELECT PA1.Nome AS 'Nome 1', PA2.NOME AS'Nome 2'
FROM
`Pessoa_Associada` AS PA1, `Pessoa_Associada` AS PA2
WHERE
`PA1`.`Endereco_ID` = `PA2`.`Endereco_ID` AND `PA1`.`Pessoa_Associada_ID` <> `PA2`.`Pessoa_Associada_ID`;

7 - Crie uma query que retorne duas coluna com os nomes das pessoas treinadoras que possuem o mesmo endereço.
 Porém, não deixe que o mesmo nome se repita na mesma linha.

SELECT PT1.Nome AS 'Treinadora 1', PT2.NOME AS'Treinadora 2'
FROM
`Pessoa_Treinadora` AS PT1, `Pessoa_Treinadora` AS PT2
WHERE
`PT1`.`Endereco_ID` = `PT2`.`Endereco_ID` AND `PT1`.`Pessoa_Treinadora_ID` <> `PT2`.`Pessoa_Treinadora_ID`;

8 - Crie uma query que retorne uma coluna com o nome dos planos outra que com a quantidade de pessoas associadas 
que possuem esse plano, nomeie como 'Pessoas Inclusas'.

SELECT
  PL.Nome_plano, COUNT(PA.Pessoa_Associada_ID) as 'Pessoas Inclusas'
  FROM
  Plano AS PL
  INNER JOIN
  Pessoa_Associada AS PA ON PL.Plano_ID = PA.Plano_ID
  GROUP BY PL.Plano_ID;

9 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo', com o nome completo das pessoas associadas 
a academia, e uma coluna com o período dessa pessoa. Utilize as tabelas Pessoa_Associada, Treino e Horario.

SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', H.Periodo
FROM
  `Pessoa_Associada` AS PA
    INNER JOIN
    Treino AS T ON PA.Pessoa_Associada_ID = T.Pessoa_Associada_ID
    INNER JOIN
    Horario AS H ON T.Horario_ID = H.Horario_ID;

    10 - Crie uma query que retorne uma coluna nomeada como 'Nome Completo' com o nome completo de 
    todas as pessoas associadas a academia e uma coluna com o nome 'Horario', mostrando o período dessa pessoa. Caso a pessoa 
    não tiver período estipulado, exiba 'LIVRE'. Utilize as tabelas Pessoa_Associada, Aula e Horario.

    SELECT  
    CONCAT (PA.Nome, ' ', PA.Sobrenome) AS 'Nome Completo', IFNULL(H.Periodo, 'LIVRE') AS 'Horario'
FROM
  `Pessoa_Associada` AS PA
    INNER JOIN
    Treino AS T ON PA.Pessoa_Associada_ID = T.Pessoa_Associada_ID
